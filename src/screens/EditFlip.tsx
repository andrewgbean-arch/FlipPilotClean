...
export default function EditFlip() {
  const theme = useTheme();
  const router = useRouter();
  const { id } = useLocalSearchParams();
  const { vehicles, updateVehicle } = useVehicleHistory();

  const flip = vehicles.find((v) => v.id === id);

  if (!flip) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
        <Text style={{ color: theme.white }}>Flip not found</Text>
      </View>
    );
  }
  ...
  const [aiPriceConfidence, setAiPriceConfidence] = useState(
    String(flip.aiPriceConfidence)
  );

  const [rarity, setRarity] = useState(flip.rarity);
  const [sellSpeed, setSellSpeed] = useState(flip.sellSpeed);
  const [condition, setCondition] = useState(flip.ai.condition);

  const [demandScore, setDemandScore] = useState(
    String(flip.market.demandScore)
  );
  ...
  const [liveScore, setLiveScore] = useState(flip.flipScore);
  ...
  const handleSave = () => {
    ...
    const aiPrice = computeAiPrice({
      ...
      rarity,
      sellSpeed,
      ai: { condition, description: flip.ai.description },
